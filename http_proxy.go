...
func newHttpProxyDailer(uri *url.URL, forward proxy.Dialer) (proxy.Dialer, error) {
	...
	d.proxyAddress = host + ":" + port
	d.proxyAddr, _ = Url2Addr(d.proxyAddress, "tcp")
...
func (d *HttpProxyDailer) validateTarget(network, address string) (net.Addr, error) {
	...
	adr, err := Url2Addr(address)

	if err != nil || adr == nil || adr.Host == "" {
		return adr, &net.AddrError{Addr: address, Err: "invalid host or port"}
	}
...
func (d *HttpProxyDailer) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	if d.forwardDail != nil {
		con, err = d.forwardDail(ctx, "tcp", d.proxyAddress)
	} else {
		dd := proxy.Direct
		con, err = dd.DialContext(ctx, "tcp", d.proxyAddress)
	}
